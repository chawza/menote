...
### Batch 1.1 — Schema fix migration
> Diesel migration to fix the notes table schema: add FK on `user_id`, make `content` NOT NULL with CHECK constraint. Regenerates `schema.rs` and updates Diesel model types accordingly. Also adds `diesel::joinable!` macro (auto-generated by the FK).
- [x] [data] Create migration: add foreign key on `notes.user_id → users(id)`, set `content NOT NULL`, add `CHECK(length(content) <= 100000)`
- [ ] ~~[data] Update `Note` model — change `content` from `Option<String>` to `String` to match new NOT NULL column~~
- [ ] ~~[data] Update `NoteDetail` DTO — change `content` from `Option<String>` to `String`~~
- [x] [data] Remove unused `std::env` import from `lib.rs`

### Batch 1.2 — Ownership checks on note mutations
> Adds `user_id` to `UpdateNote` DTO so ownership can be verified. Updates `update_note` and `delete_note` commands to filter by `user_id` and return `Unauthorized` error if the note doesn't belong to the requesting user. Uses the existing `AppError::unauthorized()` helper. Requires frontend to pass `userId` with update/delete calls. Regenerates bindings.
- [ ] [security] Add `user_id` field to `UpdateNote` struct
- [ ] [security] `update_note`: filter by both `id` and `user_id`, return `AppError::unauthorized()` if no match
- [ ] [security] `delete_note`: accept `user_id` param, filter by both `note_id` and `user_id`, return `AppError::unauthorized()` if no match
- [ ] [security] Update frontend calls to pass `userId` for update/delete operations

### Batch 1.3 — Database indexes
> Simple performance migration. Adds composite indexes on `notes(user_id, created_at DESC)` for the `get_notes` query. No code changes beyond the migration file and schema regeneration.
- [ ] [data] Create migration: add index `idx_notes_user_created` on `notes(user_id, created_at)`
...
- [ ] [feat] Add `get_notes_count(user_id)` helper to count total notes for a user
- [ ] [feat] Create `NoteListResult` DTO with `notes: Vec<NoteDetail>` and `total_count: i64`
- [ ] [feat] Update `get_notes` command signature to accept `user_id`, `limit: Option<i32>`, `offset: Option<i32>`
- [ ] [feat] Update frontend to handle paginated response (`NoteListResult` instead of `Vec<NoteDetail>`)
