...
- [ ] [data] Create migration: add foreign key on `notes.user_id → users(id)`, set `content NOT NULL`, add `CHECK(length(content) <= 100000)`
...
- [ ] [data] Remove unused `std::env` import from `lib.rs`
